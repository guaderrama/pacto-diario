document.addEventListener('DOMContentLoaded', () => {
    ...
    const firebaseConfig = {
        ...
        storageBucket: "pacto-diario.firebasestorage.app",
    ...
    // Nueva función auxiliar para llamadas a Firebase Functions
    async function callFirebaseFunction(functionName, method, body = null, requiresAuth = true) {
        let headers = {};
        if (method === 'POST') {
        ...
        const response = await fetch(`${functionsBaseUrl}/${functionName}`, options);
        const data = await response.json();

        if (!response.ok || data.error) {
            throw new Error(data.error || 'Error desconocido al llamar a la función.');
        }
        return data;
    }
    ...
    async function loadUserProfile() {
        try {
            const data = await callFirebaseFunction('getUserProfile', 'GET');
            if (data.profile) {
    ...
    profileForm.addEventListener('submit', async e => {
        ...
        try {
            const data = await callFirebaseFunction('setUserProfile', 'POST', { userName, partnerName, userLoveLanguage, partnerLoveLanguage });
            profileSaveMessage.textContent = data.message;
            setTimeout(() => profileSaveMessage.textContent = '', 3000);
            userNameDisplay.textContent = userName;
        } catch (err) {
            profileSaveMessage.textContent = `Error: ${err.message}`;
        }
    });
    ...
    generarGuiaBtn.addEventListener('click', async () => {
        ...
        try {
            const data = await callFirebaseFunction('generateLogosResponse', 'POST', { topic: userTopic }, false); // No requiere autenticación
            
            if (!data.story || !data.keyword || !data.fact || !data.prayer) {
                throw new Error('Respuesta incompleta de Logos: faltan campos.');
            }
            ...
            logosResponse.innerHTML = `<p class="text-red-500 p-4">Error: No se pudo obtener la guía. ${error.message}</p>`;
        }
    });
    ...
    async function setDailyContent() {
        ...
        try {
            const data = await callFirebaseFunction('getDailyJourneyContent', 'GET');
            if (!data.title || !data.content) {
                throw new Error('Contenido incompleto del Viaje Diario.');
            }
            ...
            dailyJourneyContent.innerHTML = `<p class="text-red-500 p-4">Error: No se pudo obtener el contenido de hoy. ${error.message}</p>`;
        }
    }
